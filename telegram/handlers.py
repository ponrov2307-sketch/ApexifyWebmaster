...
def register_handlers():
    ...
    def add_stock(message):
        ...
        try:
            ...
            if len(parts) != 4:
                bot.reply_to(message, "❌ รูปแบบผิด! กรุณาพิมพ์: <code>/add [ชื่อหุ้น] [จำนวน] [ราคา]</code>\nเช่น: <code>/add AAPL 10 150</code>", parse_mode='HTML')
                return
            ...
            bot.reply_to(message, f"✅ เพิ่มหุ้น <b>{ticker}</b> จำนวน {shares} หุ้น (ต้นทุน ${cost}) ลงในพอร์ตเรียบร้อยแล้ว!\nเปิดดูในหน้า Web ได้เลยครับ", parse_mode='HTML')
